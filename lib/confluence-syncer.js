...
async function sync() {
    try {
        ...
        await syncPages(home, siteName, localPages, assetRenderer);
 ...
 */
async function syncPages(home, siteName, localPages, renderer) {
    // TODO: this is a HACK, fix it
    localPages.forEach(lp => lp.title = `${siteName}: ${lp.title}`);
    // Filter out the paths that contain extra directories and create them first
    const decisions = localPages.find(page => page.title === `${siteName}: decisions`);
    const reviews = localPages.find(page => page.title === `${siteName}: reviews`);
