...
async function syncPages(home, localPages, renderer) {
    // first sync the parent pages
    const parentPage = localPages.find(page => page.title === 'decisions');
    parentPage.parentPageId = home; 
    await parentPage.sync(renderer, confluence);
...
async function union(parentPageId, localPages) {
    ...
    for (let localPage of localPages) {
        // TODO: only assign if it doesn't already have a parentPageId
        logger.info(`parent page title: ${localPage.parentPageTitle}`);
        if (localPage.parentPageTitle) {
            const foundParentPageId = await findParentPage(localPage.parentPageTitle);
