 ...
 */
// function traverse2(repo_url, nav, directoryPath, pages = []) {
...
//             traverse2(repo_url, nav, fullPath, pages);
//         } else if (entry.isFile() && path.extname(entry.name) === '.md') {
//             // const page = getPage(repo_url, pageTitle, path.resolve(directoryPath, 'docs', pagePath));
//             const pageTitle = entry.name.substring(0, entry.name.length-3);
//       
//             const page = getPage(repo_url, pageTitle, path.resolve(directoryPath, fullPath));
//             if (page) {
//                 pages.push(page);
//             }
...
function traverse(repo_url, nav, basePath, pages = []) {
    // find all md files in the basePath
    nav.forEach((item) => {
        ...
        if (Array.isArray(pagePath)) {
            ...
            if (relPath.includes('/')) {
                ...
                // TODO: reconstruct pagePath for parent page
                ...
                const newParent = LocalPage(newParentTitle, new Meta(repo_url, `docs/${newParentFile}`, sha));
                pages.push(newParent);
                if (page) {
                    page.setParentPageTitle(newParentTitle);
                }
            }
            if (page) {
                pages.push(page);
...
function getPage(repo_url, title, pagePath, titlePrefix = config.confluence.titlePrefix) {
    ...
    logger.info(`creating LocalPage with pagePath: ${pagePath}, and relPath: ${relPath}`);
...
function getContext(basePath = '.') {
    ...
    // const pages = traverse2(repo_url, nav, basePath);
