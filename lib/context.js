...
function traverse2(repo_url, basePath, pages = []) {
    // [ ] recursively list .md pages in /docs
    const paths = fs.readdirSync(basePath);
    // for await (const path of paths) {
    paths.forEach(path => {
        const isDirectory = fs.statSync(path).isDirectory();
        if (isDirectory) {
            traverse2(repo_url, basePath, pages);
        }
        if (path.endsWith('.md')) {
            logger.info(`found markdown file: ${path}`);
        }
    });
}
...
function traverse(repo_url, nav, basePath, pages = []) {
    // TODO: take out the dependency on nav. Just look in /path/to/docs/ to find files
 ...
 */
async function getContext(basePath = '.') {
    const { nav, repo_url, site_name } = loadConfig(basePath);

    await traverse2(repo_url, path.resolve(basePath, 'docs'));
