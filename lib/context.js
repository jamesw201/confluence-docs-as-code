...
function traverse2(repo_url, basePath, pages = []) {
    ...
    logger.info(`basePath: ${path.resolve(process.cwd(), 'docs')}`);
    const files = readdirSync(path.resolve(process.cwd(), 'docs'), []);
    logger.info(`files: ${files}`);
    files.forEach(tpath => logger.debug(`file: ${tpath}, pages: ${pages}`));
    logger.debug(`repo_url: ${repo_url}`);
    // for await (const path of paths) {
    // paths.forEach(filePath => {
    //     const fullPath = basePath === '.' ? path.resolve(basePath, filePath) : basePath
    //     const isDirectory = fs.statSync(fullPath).isDirectory()
    //     if (isDirectory) {
    //         traverse2(repo_url, fullPath, pages)
    //     }
