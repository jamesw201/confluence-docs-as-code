...
class LocalPage extends Page {
    ...
    set parentPageTitle(title) {
        this._parentPageTitle = title;
    }
    /**
    ...
    */
     ...
     */
    async sync(renderer, confluence, initials = '') {
        // Render page and attachments
        await this.render(renderer);
        // Publish page
        this.page.title = initials !== '' ? `${initials}-${this.title}` : this.title;
        logger.info(`creating page: ${this.title}`);
        const created = await confluence.createPage(this);
        logger.info(`Created Page: [${created.id}] ${created.title}`);
        // Publish attachments
        for (const attachment of this.attachmentFiles) {
            await confluence.createAttachment(created.id, attachment);
            logger.info(`Attached file ${attachment}" to page [${created.id}] ${created.title}`);
        }
        return created;
    }
}
