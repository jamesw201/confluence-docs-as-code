...
class ConfluenceSdk {
    ...
    async createPage(page) {
        ...
        // if (response.data?.message?.includes('A page with this title already exists')) {
        //     logger.info(`trying to createPage, ${page.title} already exists`)
        //     // TODO: if it already exists, get it and update it
        //     const remotePage = await this.findPage(page.title)
        //     // logger.info(`found remote page. Updating: ${remotePage.title}`)
    ...
    async updatePage(remotePage) {
        ...
        logger.info('updating page through updatePage');
    ...
    validateResponse({ status, statusText, data }, validStatuses = [200]) {
        if (!validStatuses.includes(status)) {
            logger.info(JSON.stringify({ status, statusText, data }, undefined, 2));
            if (data?.message?.includes('A page with this title already exists')) {
                logger.info('A page with this title already exists. Updating page');
            }
